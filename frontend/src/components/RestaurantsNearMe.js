    ...
    <div className='flex'>
      <div className='bg-slate-200 ml-12'>
        <div className='flex  w-5/6 ml-auto mr-auto'>
          ...
          <div className='mt-8'><AiFillEdit size={20} className={(!editState) ? 'mx-1.5 hover:scale-110 text-blue-700' : 'hidden'} onClick={() => setEditState(!editState)}/></div>
          <div className='mt-8'><AiFillCheckCircle size={20} className={(editState) ? 'mx-1.5 hover:scale-110 text-green-700' : 'hidden'} onClick={(e) => {updateMeters(e)}}/></div>
        </div>
        ...
        <div className='ml-auto mr-12 mt-12'>
          <Map restaurants={restaurants} position={ [46.549567,15.649397]} width={"700px"} height={"500px"} oneMarker={false}/>
